Autonomous execution of the ralph contract at plans/cloudflare-otel-parity/002-tail-sampling/.
...
5. Honor §5 guardrails (additive only — no `sampling` config means SimpleSpanProcessor behaviour byte-identical; record-all invariant — default head sampler NEVER returns NOT_RECORD; tail decision + export in ctx.waitUntil; bounded buffer; no new runtime dep), §6 escalation (Opus on tasks 001/002/003/004 or attempts >= escalate_after), §7 abort.
