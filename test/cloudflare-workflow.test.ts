...
const { mockExtract, mockInject, mockSpans, createMockSpan, activeCtxStack } = vi.hoisted(() => {
	type MockSpan = {
		name: string;
	...
	return {
		mockExtract: vi.fn((_ctx: unknown, _carrier: unknown) => ({ extracted: true })),
		mockInject: vi.fn(
			(
				_ctx: unknown,
				carrier: Record<string, string>,
				setter?: {
					set: (c: Record<string, string>, k: string, v: string) => void;
				},
			) => {
				if (setter) {
...
function createMockStep() {
	return {
		do: vi.fn(
			(_name: string, configOrCallback: unknown, maybeCallback?: () => Promise<unknown>) => {
...
describe("instrumentWorkflow", () => {
	...
	it("wraps step.do with span using user's step name", async () => {
		...
		expect(userSpan!.opts).toEqual(expect.objectContaining({ kind: SpanKind.INTERNAL }));
	...
	it("forwards WorkflowStepConfig to original step.do", async () => {
		...
		const userCall = step.do.mock.calls.find((c: unknown[]) => c[0] === "with-config");
	...
	it("records error and re-throws on failure", async () => {
		...
		await expect(instance.run({ payload: {} }, step)).rejects.toThrow("step failed");
	...
	it("creates a root workflow.run span wrapping the entire run", async () => {
		...
		expect(rootSpan!.opts).toEqual(expect.objectContaining({ kind: SpanKind.INTERNAL }));
	...
	it("nested step.do spans become children of outer step span", async () => {
		...
		class TestWorkflow {
			env = {};
			async run(_event: unknown, step: Record<string, unknown>) {
				await (step.do as CallableFunction)("outer", async () =>
	...
	it("root span records error when run() throws", async () => {
		...
		await expect(instance.run({ payload: {} }, step)).rejects.toThrow("run crashed");
