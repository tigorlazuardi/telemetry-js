...
describe("resolveSignalEndpoint", () => {
	...
	describe("priority ordering", () => {
		...
		it("uses OTEL_EXPORTER_OTLP_TRACES_ENDPOINT env (full URL) as highest priority", () => {
			...
			expect(resolveSignalEndpoint("traces", config)).toBe("https://env-traces.example.com/full");
		});
		...
		it("uses OTEL_EXPORTER_OTLP_LOGS_ENDPOINT env for logs", () => {
			...
			expect(resolveSignalEndpoint("logs", baseConfig)).toBe("https://env-logs.example.com/full");
		});
