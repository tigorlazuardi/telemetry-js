...
describe("instrumentFetch", () => {
	...
	describe("span", () => {
		...
		it("sets ERROR status on >= 500", async () => {
			const mockFetch = vi.fn().mockResolvedValue(new Response("error", { status: 502 }));
			const tracedFetch = instrumentFetch(mockFetch);
	...
	describe("error handling", () => {
		it("sets ERROR + recordException on fetch failure, re-throws", async () => {
			...
			await expect(tracedFetch("https://api.example.com/data")).rejects.toThrow("network error");
	...
	describe("response traceparent", () => {
		it("records http.response.traceparent attr when present", async () => {
			...
			expect(mockSpan.setAttribute).toHaveBeenCalledWith("http.response.traceparent", traceparent);
		});
	...
	describe("logging", () => {
		...
		it("emits ONE logger.error on fetch failure", async () => {
			...
			await expect(tracedFetch("https://api.example.com/data")).rejects.toThrow(
				"connection refused",
			);

			expect(logger.error).toHaveBeenCalledTimes(1);
			const [message, attrs] = logger.error.mock.calls[0];
			expect(message).toMatch(/^GET https:\/\/api\.example\.com\/data FAILED \d+ms$/);
			expect(attrs).toMatchObject({
				"http.method": "GET",
				"http.url": "https://api.example.com/data",
				"http.duration_ms": expect.any(Number),
	...
	describe("passthrough", () => {
		...
		it("caller can read response body without logger", async () => {
			const mockFetch = vi.fn().mockResolvedValue(new Response("plain text"));
			const tracedFetch = instrumentFetch(mockFetch);
