...
describe("parseEnvResourceAttributes", () => {
	it("parses comma-separated key=value pairs", () => {
		const result = parseEnvResourceAttributes({
			OTEL_RESOURCE_ATTRIBUTES: "service.namespace=my-team,deployment.environment.name=production",
		});
...
describe("buildResource", () => {
	...
	it("uses config.serviceName when provided", () => {
		withTTY(false, () => {
			const { resource } = buildResource({ serviceName: "my-api" }, []);
			...
			const { resource } = buildResource(
				{
					serviceName: "test",
					env: {
						OTEL_RESOURCE_ATTRIBUTES: "deployment.environment.name=staging,service.namespace=my-ns",
					},
				},
				[],
			);
	...
	it("emits warning when deployment.environment.name is not set and not TTY", () => {
		withTTY(false, () => {
			...
			const envWarning = warnings.find((w) => w.includes("deployment.environment.name is not set"));
	...
	it("sets deployment.environment.name to 'local' when TTY", () => {
		withTTY(true, () => {
			const { resource, warnings } = buildResource({ serviceName: "test" }, []);
			expect(resource.attributes[ATTR_DEPLOYMENT_ENVIRONMENT_NAME]).toBe("local");
			const envWarning = warnings.find((w) => w.includes("deployment.environment.name is not set"));
	...
	it("emits warning when service.namespace is not set and not TTY", () => {
		withTTY(false, () => {
			const { warnings } = buildResource({ serviceName: "test" }, []);
			const nsWarning = warnings.find((w) => w.includes("service.namespace is not set"));
	...
	it("merges runtime detector attributes into resource", () => {
		...
		withTTY(true, () => {
			const { resource } = buildResource({ serviceName: "test" }, [customDetector]);
	...
	it("does not emit service.name warning when serviceName is set", () => {
		withTTY(true, () => {
			...
			const svcWarning = warnings.find((w) => w.includes("service.name resolved to"));
